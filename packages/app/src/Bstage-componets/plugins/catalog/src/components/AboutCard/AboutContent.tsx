...
export function AboutContent(props: AboutContentProps) {
  ...
  return (
    <Grid container style={{position: 'relative'}}>
      <AboutField style={styles.aboutHeading} label="Description" gridSizes={{ xs: 12 }}>
        <MarkdownContent
          className={classes.description}
          content={entity?.metadata?.description || 'No description'}
        />
      </AboutField>
      {/* <img style={{width: 300, height: 300, position: 'absolute', right: 50}} src={googleIcon} alt="Google logo" /> */}
      <AboutField
        label="Owner"
        value="No Owner"
        gridSizes={{ xs: 12 }}
      ...
      >
        {ownedByRelations.length > 0 && (
          <EntityRefLinks style={{color: colors.TEXT_COLOR_2}} entityRefs={ownedByRelations} defaultKind="group" />
        )}
      </AboutField>
