...
export const PodsTable = ({ pods, extraColumns = [] }: PodsTablesProps) => {
  ...
  const headerStyle = {backgroundColor: colors.PRIMARY_COLOR_2};
  const cellStyle = {backgroundColor: colors.PRIMARY_COLOR_2};
  ...
  if (extraColumns.includes(RESOURCE_COLUMNS)) {
    ...
    styledColumns = columns.map(item=> ({...item, cellStyle, headerStyle}))
  }
  ...
  return (
    <div style={tableStyle}>
      <Table
        options={{ paging: false, search: false, emptyRowsWhenPaging: false }}
        style={{backgroundColor: colors.PRIMARY_COLOR_2}}
        data={pods as Pod[]}
