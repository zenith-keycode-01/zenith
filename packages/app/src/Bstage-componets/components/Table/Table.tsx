...
function convertColumns<T extends object>(
  ...
  return columns.map(column => {
    const headerStyle: React.CSSProperties = column.headerStyle ?? {backgroundColor: 'white', color: 'black', height: '50px'};

    let cellStyle = {color: colors.PRIMARY_TEXT_COLOR, background: colors.WHITE, fontSize: '14px', fontWeight: 500};
  ...
  return (
    <Box className={tableClasses.root}>
      ...
      <MTable<T>
        ...
        style={{ width: '100%', backgroundColor: colors.PRIMARY_COLOR_2, color: colors.BACKGROUND_COLOR, borderRadius: 10, overflow: 'hidden'}}
        localization={{
          toolbar: { searchPlaceholder: 'Search', searchTooltip: 'Filter' },
        }}
        {...restProps}
      />
    </Box>
