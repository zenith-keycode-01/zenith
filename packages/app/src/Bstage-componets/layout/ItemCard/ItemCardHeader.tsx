...
export function ItemCardHeader(props: ItemCardHeaderProps) {
  ...
  return (
    <Box style={{backgroundColor: colors.PRIMARY_COLOR_2, padding: '10px 10px'}}>
      {subtitle && (
        <Typography variant="subtitle2" component="h3" style={{color: '#bbbbb3'}}>
          {subtitle}
        </Typography>
      )}
      {title && (
        <Typography variant="h6" component="h4" style={{color: '#e9e9e7'}}>
          {title}
        </Typography>
      )}
      {children}
