  ...
  makeStyles<BackstageTheme>(
    theme => ({
      ...
      highlightable: {
        '&:hover': {
          // borderRadius: 50,
          background: colors.PRIMARY_COLOR_6,
      ...
      selected: {
        ...
        color: colors.PRIMARY_TEXT_COLOR,
        borderRadius: 50
          // borderBottom: `solid ${sidebarConfig.selectedIndicatorWidth}px ${theme.palette.navigation.indicator}`,
  ...
  return (
    <Link
      ...
      style={{
        ...
        // ...(isActive ? { borderColor: 'black', } : undefined),
      }}
  ...
  const openContent = (
    <>
      <Box data-testid="login-button" className={classes.iconContainer}>
        {imageUrl?.length? <img src={imageUrl}  style={{width:'40px', height: '40px', borderRadius:"20px", overflow: 'hidden'}}/>:itemIcon}
      </Box>
      {text && (
        <Typography
          variant="subtitle2"
          ...
          style={imageUrl?.length && {paddingLeft:'10px'}}
        >
          {text}
        </Typography>
      )}
